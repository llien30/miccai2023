...
from .data_utils import get_nearest_pose_ids, random_crop
from .llff_data_utils import batch_parse_llff_poses, load_llff_data


class LLFFMedDataset(Dataset):
    def __init__(self, args, mode, scenes=(), random_crop=True, **kwargs):
        ...
        for train_video in self.args.train_videos:
            folder_path = os.path.join(args.rootdir, "data/", train_video)
            scenes = os.listdir(folder_path)
            # print(f"Scenes for {train_video}: {all_scenes}")
            # if len(scenes) > 0:
